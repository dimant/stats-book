...
to arrive at the distribution of the arithmetic mean $\bar{X} \thicksim N(\mu, \frac{\sigma^2}{n})$. Note that we are dividing by $n$. This means that with greater $n$ the variance of the mean gets smaller or in other words, the more data we observe, the more sure we are about estimating the mean.  This would be sufficient to test our hypothesis $H_0: \mu = 30$. In textbooks however you will find a different definition of the \hbox{Z-Test}:
